...
export default {
  ...
  data(){
    ...
    return{
      ...
      editFormRules:{
        email: [
          { required: true, message: '请输入邮箱', trigger: 'blur' }, // 失去焦点时触发
          { min: 4, max: 18, message: '邮箱长度在6 到 16 个字符', trigger: 'blur' },
          { validator: checkEmail, trigger: 'blur' }
        ],
        mobile: [
          { required: true, message: '请输入手机', trigger: 'blur' }, // 失去焦点时触发
          { min: 11, max: 11, message: '手机长度在6 到 15 个字符', trigger: 'blur' },
      ...
      addFormRules:{
        ...
        password: [
          ...
          { min: 3, max: 15, message: '密码长度在6 到 15 个字符', trigger: 'blur' }
        ],
        email: [
          { required: true, message: '请输入邮箱', trigger: 'blur' }, // 失去焦点时触发
          { min: 4, max: 18, message: '邮箱长度在6 到 16 个字符', trigger: 'blur' },
  ...
  methods:{
    ...
    // 点击按钮，添加新用户
    addUser(){
      console.log("点击按钮，添加新用户")
      this.$refs.addFormRef.validate( async valid => {
        console.log("自定义表单校验结果:",valid)
        if(!valid){
          // 预校验失败
    ...
    async removeUserById(id){
      ...
      // 如果确认删除，贼返回值为字符串“confirm”
      // 如果取消删除，贼返回值为字符串“cancle”
